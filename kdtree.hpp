//
// https://github.com/jkflying/bucket-pr-kdtree/tree/master
//
 ...
 *
 * // setup
 * using tree_t = kdtree::KDTree<std::string, 2>;
 * using point_t = Eigen::Vector<double, 2>;
 * tree_t tree;
...
namespace kdtree {
    ...
    template<typename Scalar, std::size_t Dimensions>
    struct L1 {
        static Scalar distance(const Eigen::Vector<Scalar, Dimensions> &location1,
                               const Eigen::Vector<Scalar, Dimensions> &location2) {
            return (location1 - location2).template lpNorm<1>();
        }
    };
    ...
    template<class Payload,
            ...
            class Distance = SquaredL2<float, 3>,
    ...
    public:
        ...
        using point_t = Eigen::Vector<Scalar, Dimensions>;
        ...
        [[nodiscard]] size_t size() const { return m_nodes[0].m_entries; }
        ...
        void splitOutstanding() {
            ...
            while (!searchStack.empty()) {
                ...
                if (m_nodes[addNode].m_splitDimension == Dimensions && m_nodes[addNode].shouldSplit() &&
                    split(addNode)) {
        ...
        class Searcher {
        public:
            explicit Searcher(const tree_t &tree) : m_tree(tree) {}
    ...
    private:
        ...
        struct Node {
            explicit Node(std::size_t capacity) { init(capacity); }
            ...
            [[nodiscard]] bool shouldSplit() const { return m_entries >= BucketSize; }
            ...
            Eigen::Vector<Range, Dimensions> m_bounds; /// bounding box of this node
